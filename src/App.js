...
class App {
  async run() {
    const stingInput = await Console.readLineAsync('덧셈할 문자열을 입력해 주세요.\n');
    ...
    // 입력값 시작 검사
    if (!stingInput || (!stingInput.startsWith('//') && !(Number(stingInput[0]) >= 0))) {
      if (stingInput.startsWith('-')){
    ...
    if (stingInput.startsWith('//')) {
      // 커스텀 구분자 문법 검사
      if (!stingInput.includes('\\n')){
        throw new Error('[ERROR] 커스텀 구분자 문법을 제대로 입력해주세요.');
      }
      // '\n' 기준으로 커스텀 구분자와 본문 분리
      [delimiterPart, numberPart] = stingInput.split('\\n');
      ...
      // 기본 구분자
      numbers = stingInput.split(/,|:/).map(str => {
        const NUM_STR = str.trim();
        const NUM = Number(NUM_STR);
